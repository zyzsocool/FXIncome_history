        ...
        for hdp in self.hdp:
            ...
            for position, position0 in zip(self.position_deal, self.position_notdeal):
                ...
                if position.asset.code in hdp.buysell.keys():
                    ...
                    position.change(face_value_delta=hdp.buysell[position.asset.code])#改变债券头寸
            ...
            for position, position0 in zip(self.position_deal, self.position_notdeal):
                ...
                if position.asset.code in hdp.buysell.keys():
                    ...
                    if hdp.buysell[position.asset.code] > 0:
                        ...
                        face_value = hdp.buysell[position.asset.code]
                        ...
                        # 3.1.1.1.3将新买入资产放入position_notdeal（为了两个list大小一致，因此face_value定为0，相当于没有买入）
                        face_value = 0
                        assetappend2 = Bond(
                            code,
                            ctype,
                    ...
                    #3.1.1.2卖出资产的情况
                        ...
                        while sellvol < 0:
                            if sellvol + position_in.face_value <= 0:#1900210卖完了还不够，就卖190210-1
                                sellvol_in = -position_in.face_value#把能卖的全部卖完
                            ...
                            pricegain = (cleanprice_market0 - cleanprice_def0) - (cleanprice_market1 - cleanprice_def1)#计算价差收入
